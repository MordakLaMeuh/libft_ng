		...
		struct s_node_params *use_ctx)
{
	...
	if ((diff = use_ctx->comp(content, node)) < 0)
	{
		if (node->left == NULL)
			...
			return (simulate_insert_child(node->left, content, use_ctx));
	}
	else if (diff > 0)
	{
		if (node->right == NULL)
			...
			return (simulate_insert_child(node->right, content, use_ctx));
	}
...
static struct s_node	*btree_try_insert_node(
		struct s_node **root,
		void *content,
		struct s_node_params *use_ctx)
{
